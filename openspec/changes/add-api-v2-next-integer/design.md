...
`wndFinancialDocuments_generateInvoiceDateAndID.php` finds the largest invoice-number property after optionally filtering by the current year and invoice series, adds one, and writes the result. Its item type and application properties are hard-coded and its read-then-write sequence is not protected against concurrent requests.
...
The maximum is calculated only for the resolved client, item type, and target property. Optional scopes add:
...
No matching positive value means the assigned value is `1`. Non-positive legacy values do not lower or otherwise change the maximum.

### Aggregate calculation

Add a dedicated helper to `RSMitemsManagement.php` that executes a database-side `MAX()` over the integer property table and joins only the supplied scope properties. Do not retrieve every matching item through `IQ_getFilteredItemsIDs()`. Resolve table names exclusively from validated property types, cast identifiers to integers, and bind request-derived scope values.

This is both more memory-efficient and easier to keep atomic than adapting the current invoice loop.

### Concurrency and persistence

Acquire a MariaDB/MySQL named advisory lock before re-reading the target value, calculating the maximum, and calling `setPropertyValueByID()`. Derive the bounded lock name from a hash of client ID, target property ID, year scope, and series scope. Always release it in a `finally` path.
...
- The four legacy generators now reuse the aggregate helper internally.
- Strong uniqueness across every RSM writer would require a later storage-level design.

## Legacy caller migration

The four existing number generators delegate to the shared MAX helper. Internal series scopes also accept a set of equality values for the existing grouped-relation case; an empty set matches nothing, and lock keys canonicalize order and duplicates. This does not extend the API JSON contract.
