...
- [x] 1.1 Add `RS_CUSTOMER_ITEM_TYPE_ID` and `RS_CUSTOMER_ITEM_ID` as nullable unsigned integer columns to `rs_tokens` in `Database/schema.sql`.
...
- [x] 2.3 Add item helper logic to find the single direct customer `identifier` dependency property for an item type using the token customer item type ID.
