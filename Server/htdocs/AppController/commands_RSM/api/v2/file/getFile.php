...
function getAskedFile()
{
    ...
    if ($enable_file_cache && count($nombres_archivo) > 0) {
        ...
        if ($file) {
            ...
            if ($RSallowDebug) returnJsonMessage(404, "Requested file does not exist");
...
function validateRequestParams($parameters)
{
    ...
    // validate that parameters include ID and propertyID
    if (!(isset($parameters["ID"]) && isset($parameters["propertyID"]))) {
        if ($RSallowDebug) returnJsonMessage(400, "Request must include query params:'ID' and 'propertyID'");
