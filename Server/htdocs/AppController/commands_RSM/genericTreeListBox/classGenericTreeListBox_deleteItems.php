...
function deleteDescendants($clientID, $parentID, $allowedItemPath, $userID, $counter = 100)
{
  ...
  if (count($allowedItemPath) > 0 && $counter >= 0) {
    ...
    if (count(getUserVisiblePropertiesIDs($allowedItemType[0], $clientID, $userID)) > 0) {
      ...
      if ($allowedItemType[1] == 0) {
        ...
        if (!empty($descendants) && !deleteDescendants($clientID, $parentID, array_merge(array($allowedItemType[0] . "," . $descendants[0]['propertyID']), $allowedItemPath), $userID, $counter)) {
          //not enough permissions to delete descendants
          return false;
        }
        //recursive delete descendants
        if (!deleteDescendants($clientID, $parentID, $allowedItemPath, $userID, $counter)) {
          //not enough permissions to delete descendants
          return false;
        }
      } else {
        ...
        foreach ($result as $item) {
          //apply recursively if recursive itemtype
          if (!empty($descendants) && array_key_exists('recursivePropertyID', $descendants[0]) && !deleteDescendants($clientID, $item['ID'], array_merge(array($allowedItemType[0] . "," . $descendants[0]['recursivePropertyID']), $allowedItemPath), $userID, $counter)) {
            //not enough permissions to delete descendants
            return false;
          }
          //recursive delete descendants
          if (deleteDescendants($clientID, $item['ID'], $allowedItemPath, $userID, $counter)) {
    ...
    if (isPropertyVisible($userID, $descendant['propertyID'], $clientID)) {
      ...
      if ($result) {
        while ($item = $result->fetch_assoc()) {
          ...
          if ($res < 0) {
            return $res; //error while updating
          }
        }
      }
    } else {
