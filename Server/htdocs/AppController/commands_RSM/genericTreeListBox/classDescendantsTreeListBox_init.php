...
foreach($destinationItemTypes as $destinationItemTypeID){
	...
	foreach($treePath as $path){
		...
		for($i=0; $i<count($path); $i++){
			$tempPath[] = array(
				'nodeID'           => '1',
				'name'             => getClientItemTypeName($path[$i]['itemTypeID'], $clientID),
				'icon'             => getClientItemTypeIcon($path[$i]['itemTypeID'], $clientID),
				'nodeItemType'     => $path[$i]['itemTypeID'],
				'parentItemType'   => ($i > 0               ) ? ($path[$i-1]['itemTypeID']) : ($parentItemTypeID),
				'parentPropertyID' => ($i > 0               ) ? ($path[$i]['propertyID']) : ('0'),
				'parentID'         => ($i > 0               ) ? ('1') : ('0'),
				'childs'           => ($i < count($path) - 1) ? ('1,' . $path[$i+1]['itemTypeID']) : ('')
			);
		}
...
foreach ($results as $result){
	if ($result['parentPropertyID']!='0' && !isPropertyVisible($RSuserID, $result['parentPropertyID'], $clientID)) {
		if (!in_array($result['nodeItemType'], $notAllowedItemtypes)) {
			$notAllowedItemtypes[] = $result['nodeItemType'];
		}
	}
}
