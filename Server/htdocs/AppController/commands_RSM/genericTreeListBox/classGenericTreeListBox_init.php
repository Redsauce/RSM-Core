...
if ($allowedItemTypesNames != '') {
    ...
    foreach ($allowedItemTypes as $allowedItemType) {
        ...
        $descendants_string = '';
        foreach ($subDescendants as $subDescendant) $descendants_string .= $subDescendant['propertyID'] . "," . $subDescendant['itemTypeID'] . "," . $subDescendant['propertyType'] . "," . base64_encode($subDescendant['propertyName']) . ";";
        
        $descendants_string = rtrim($descendants_string, ";");

        if (!array_key_exists($allowedItemType, $descendantsArray)) $descendantsArray[$allowedItemType] = $descendants_string;

        //get Icon
        if (!array_key_exists($allowedItemType, $iconsArray)) $iconsArray[$allowedItemType] = getClientItemTypeIcon($allowedItemType, $clientID);
    ...
    if ($res) {
        while ($row = $res->fetch_assoc()) {
            ...
            if (!array_key_exists($row['RS_ITEMTYPE_ID'], $descendantsArray)) $descendantsArray[$row['RS_ITEMTYPE_ID']] = $descendants_string;

            //get Icon
            if (!array_key_exists($allowedItemType, $iconsArray)) $iconsArray[$row['RS_ITEMTYPE_ID']] = bin2hex($row['RS_ICON']);
        }
    }
}

$descendantsString = '';
foreach ($descendantsArray as $availableItemType => $descendantsForItem) $descendantsString .= $availableItemType . "=>" . $descendantsForItem . ":";

$iconsString = '';
foreach ($iconsArray as $availableItemType => $iconForItem) $iconsString .= $availableItemType . "=>" . $iconForItem . ":";
