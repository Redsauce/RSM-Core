...
// Prueba con MariaDB local: compara el codigo legacy contra el actual.
// La base temporal se crea y se elimina en cada ejecucion.

$root = dirname(__DIR__);
$currentPath = $root . '/Server/htdocs/AppController/commands_RSM/utilities/RSMitemsManagement.php';
...
function compareScenario($name, $legacy, $current) {
    $legacyRows = normalizeRows($legacy);
    $currentRows = normalizeRows($current);
    if ($legacyRows !== $currentRows) {
        fwrite(STDERR, "FAIL: scenario {$name} produced different responses\n");
        fwrite(STDERR, "Legacy count: " . count($legacyRows) . ", current count: " . count($currentRows) . "\n");
        fwrite(STDERR, "Legacy sample: " . json_encode(array_slice($legacyRows, 0, 3)) . "\n");
        fwrite(STDERR, "Current sample: " . json_encode(array_slice($currentRows, 0, 3)) . "\n");
        exit(1);
    }
    echo "PASS {$name}: " . count($currentRows) . " rows\n";
}
...
foreach ($scenarios as $scenario) {
    ...
    $legacyRows = Legacy_getFilteredItemsIDs(5, 7, $filters, $returns, $orderBy, $translate, $limit, $ids, $join, $returnOrder, $allowFile, '', false);
    $currentRows = getFilteredItemsIDs(5, 7, $filters, $returns, $orderBy, $translate, $limit, $ids, $join, $returnOrder, $allowFile, '', false);
    compareScenario($name, $legacyRows, $currentRows);
}

$mysqli->query('DROP DATABASE `' . $dbName . '`');
echo "dynamic item join MariaDB differential tests passed\n";
